...
extension Array where Element: PPView {
     ...
     */
    @discardableResult
    public func set(_ edge: PPAttribute, of value: CGFloat, priority: PPPriority = .required) -> [NSLayoutConstraint]? {
     ...
     */
    @discardableResult
    public func set(_ edges: PPAttribute..., of value: CGFloat, priority: PPPriority = .required) -> [PPMultipleConstraints] {
        var constraintsArray: [PPMultipleConstraints] = []
        for view in self {
     ...
     */
    @discardableResult
    public func spread(_ axis: PPAxis, stretchEdgesToSuperview: Bool = false, offset: CGFloat = 0, priority: PPPriority = .required) -> [NSLayoutConstraint]? {
    ...
    /**
     ...
     - parameter priority: Constraint's priority (default is *.required*).
     - returns: Array of PPAxisConstraints - see definition (discardable)
     */
    @discardableResult
    public func layoutToSuperview(axis: PPAxis, offset: CGFloat = 0, priority: PPPriority = .required) -> [PPAxisConstraints]? {
        ...
        var constraints: [PPAxisConstraints] = []
        for (first, second) in zip(firstConstraints, secondConstraints) {
            constraints.append(PPAxisConstraints(first: first, second: second))
        }
        return constraints
    }
     ...
     */
    @discardableResult
    public func layoutToSuperview(_ edge: PPAttribute, ratio: CGFloat = 1, offset: CGFloat = 0, priority: PPPriority = .required) -> [NSLayoutConstraint]? {
        guard isValidForPapaLayout else {
            return nil
        }
     ...
     */
    @discardableResult
    public func layout(_ firstEdge: PPAttribute? = nil, to anchorEdge: PPAttribute, of anchorView: PPView, ratio: CGFloat = 1, offset: CGFloat = 0, priority: PPPriority = .required) -> [NSLayoutConstraint]? {
        guard isValidForPapaLayout else {
            return nil
        }
        
        let edge: PPAttribute
    ...
    /**
     ...
     - parameter offset: Additional offset from that can be applied to the constraints (default is 0).
     - parameter priority: Constraints' priority (default is *.required*).
     - returns: Array of applied constraints, each element is of type PPMultipleConstraints - see definition (discardable)
     */
    @discardableResult
    public func layout(_ edges: PPAttribute..., to anchorView: PPView, ratio: CGFloat = 1, offset: CGFloat = 0, priority: PPPriority = .required) -> [PPMultipleConstraints]? {
        guard !edges.isEmpty && isValidForPapaLayout else {
            return nil
        }
        // Avoid duplicities
        let uniqueEdges = Set(edges)
        var result: [PPMultipleConstraints] = []
        for view in self {
            var multipleConstraints: PPMultipleConstraints = [:]
    ...
    var isValidForPapaLayout: Bool {
        ...
        for view in self {
            guard view.isValidForPapaLayout else {
